...
>
> **Kernel version and TIOCSTI:** On kernels < 5.4, bwrap's minimal `/dev` has broken pty allocation (the user-namespace devpts gets `ptmxmode=000`). The sandbox works around this by binding the host's entire `/dev` into the sandbox (`BIND_DEV_PTS=auto`), which exposes `/dev/pts` — the host's pseudo-terminal devices. On kernels < 6.2, this enables the **TIOCSTI escape**: a sandboxed process can use the `TIOCSTI` ioctl to inject keystrokes into any same-user terminal outside the sandbox. Upgrading to **kernel ≥ 5.4** eliminates the need for host `/dev` exposure (bwrap's minimal `/dev` provides working ptys). Upgrading to **kernel ≥ 6.2** disables TIOCSTI entirely (`CONFIG_LEGACY_TIOCSTI=n` by default), making host `/dev` exposure safe even when forced via `BIND_DEV_PTS=true`. The current gizmo nodes run kernel 4.15, which requires host `/dev` and is vulnerable to TIOCSTI.
