...
Hides SSH keys, cloud credentials, GPG keys, and environment secrets from AI coding agents while letting them do their job. Three backends (bubblewrap, firejail, Landlock), six built-in agent profiles (Claude Code, Codex, Gemini, Aider, OpenCode, pi-mono) with a one-line recipe for adding more, zero containers. Network egress is enforced by the Linux kernel at the network-namespace boundary (four [`NETWORK_FILTER_MODE`](reference/network-filter.md#modes) values — `open`/`filtered`/`proxied`/`isolated`), not by the agent's own permission system: a jailbroken agent cannot reach destinations outside the configured policy regardless of what its CLI tooling allows.
