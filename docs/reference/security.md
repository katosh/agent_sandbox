...
| Agent silently rewrites in-tree `.git/`, `.config/agent-sandbox/`, or `.claude/projects/` | `WRITABLE_TREE_RO_PATHS` re-overlays each matching descendant of a writable root as `--ro-bind` (bwrap) / `--read-only` (firejail). Defaults cover git state, the sandbox's own per-project config, and Claude Code session logs; nested matches (submodules) are picked up via a path-suffix `find -prune` walk | **Hard** (bwrap/firejail) / N/A (Landlock — additive rules cannot make a subdir read-only when its parent is writable; warning emitted) |
