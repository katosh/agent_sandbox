...
| `BLOCKED_FILES` entry doesn't exist on host (silent no-op) | At config-load, `sandbox-exec.sh` materializes a zero-byte placeholder for every missing entry under writable parents and refuses to start (with the full list of failing entries) when materialization fails. Closes a pre-v0.11.1 gap where the bwrap/firejail backends skipped the bind for missing entries, leaving the path unenforced; also prevents bwrap's own `ensure_file → creat()` from leaving a host stub during mount setup. See [#73](https://github.com/katosh/agent_sandbox/issues/73) | **Hard** (bwrap/firejail; fail-loud or materialize) / N/A (Landlock) |
