...
# ── D01: Project-tree dotdirs (.git etc.) are read-only ──
# Default WRITABLE_TREE_RO_PATHS overlays .git, .config/agent-sandbox,
# and .claude/projects read-only inside the project tree even though
# the project dir is bound writable. Mount-namespace backends only
# (bwrap, firejail) — Landlock cannot make a subdir read-only when the
# parent is writable, and the surrounding warning is checked
# elsewhere.
if has_mount_ns; then
    local _gitdir="$PROJECT_DIR/.git"
    local _git_pre_existing=true
    if [[ ! -d "$_gitdir" ]]; then
        _git_pre_existing=false
        mkdir -p "$_gitdir"
        echo "[core]" > "$_gitdir/config"
        _TEST_TEMP_FILES+=("$_gitdir")
    fi
    if sandbox bash -c "echo 'tampered = true' >> '$_gitdir/config' 2>&1; echo EXIT=\$?"; then
        if echo "$OUTPUT" | grep -qE "Read-only file system|Permission denied|EXIT=[1-9]"; then
            pass "D01: Project-tree .git/ is read-only inside sandbox"
        else
            fail "D01: Project-tree .git/config write succeeded — should be RO" "$OUTPUT"
        fi
    else
        pass "D01: Project-tree .git/ blocked write"
    fi
    # Confirm reads still work — the overlay must not hide content.
    if sandbox bash -c "cat '$_gitdir/config' 2>&1; echo EXIT=\$?"; then
        if echo "$OUTPUT" | grep -q '\[core\]'; then
            pass "D01: Project-tree .git/ still readable inside sandbox"
        else
            fail "D01: Project-tree .git/config is hidden (should be RO, not blocked)" "$OUTPUT"
        fi
    else
        fail "D01: Reading .git/config failed inside sandbox" "$OUTPUT"
    fi
    if ! $_git_pre_existing; then
        rm -rf "$_gitdir"
    fi
else
    skip "D01: WRITABLE_TREE_RO_PATHS requires bwrap or firejail"
fi

# ── D02: Project-tree .claude/projects/ is read-only ──
if has_mount_ns; then
    local _claudedir="$PROJECT_DIR/.claude/projects"
    mkdir -p "$_claudedir"
    echo '{"existing": "session"}' > "$_claudedir/session.jsonl"
    _TEST_TEMP_FILES+=("$PROJECT_DIR/.claude")
    if sandbox bash -c "echo '{\"tamper\": true}' > '$_claudedir/EXFIL.jsonl' 2>&1; echo EXIT=\$?"; then
        if echo "$OUTPUT" | grep -qE "Read-only file system|Permission denied|EXIT=[1-9]"; then
            pass "D02: Project-tree .claude/projects/ blocks writes of new files"
        else
            fail "D02: Project-tree .claude/projects/ accepted EXFIL.jsonl write" "$OUTPUT"
        fi
    else
        pass "D02: Project-tree .claude/projects/ blocked write"
    fi
    rm -rf "$PROJECT_DIR/.claude"
else
    skip "D02: WRITABLE_TREE_RO_PATHS requires bwrap or firejail"
fi

# ── D03: Nested .git/ (submodule-shape) is read-only ──
# A nested .git/ deep in the tree must also pick up the overlay so
# `git -C sub config …` can't be misused to silently modify a
# submodule's state.
if has_mount_ns; then
    local _nestdir="$PROJECT_DIR/sub/nested/.git"
    mkdir -p "$_nestdir"
    echo "[core]" > "$_nestdir/config"
    _TEST_TEMP_FILES+=("$PROJECT_DIR/sub")
    if sandbox bash -c "echo 'tampered = true' >> '$_nestdir/config' 2>&1; echo EXIT=\$?"; then
        if echo "$OUTPUT" | grep -qE "Read-only file system|Permission denied|EXIT=[1-9]"; then
            pass "D03: Nested project-tree .git/ is read-only"
        else
            fail "D03: Nested .git/config write succeeded — overlay missed nested match" "$OUTPUT"
        fi
    else
        pass "D03: Nested project-tree .git/ blocked write"
    fi
    rm -rf "$PROJECT_DIR/sub"
else
    skip "D03: WRITABLE_TREE_RO_PATHS requires bwrap or firejail"
fi

# ── D04: User can opt out via WRITABLE_TREE_RO_PATHS=() ──
# A user who explicitly wants .git writable (e.g. to git commit from
# inside the sandbox) can clear the array in conf.d/. This guards
# against regressions where the overlay becomes mandatory.
if has_mount_ns; then
    local _gitdir2="$PROJECT_DIR/.git"
    local _git_pre_existing2=true
    if [[ ! -d "$_gitdir2" ]]; then
        _git_pre_existing2=false
        mkdir -p "$_gitdir2"
        echo "[core]" > "$_gitdir2/config"
        _TEST_TEMP_FILES+=("$_gitdir2")
    fi
    local _opt_out_conf="$HOME/.config/agent-sandbox/conf.d/test-dotdir-opt-out-$$.conf"
    _TEST_TEMP_FILES+=("$_opt_out_conf")
    mkdir -p "$HOME/.config/agent-sandbox/conf.d"
    echo 'WRITABLE_TREE_RO_PATHS=()' > "$_opt_out_conf"
    if sandbox bash -c "echo 'tampered = true' >> '$_gitdir2/config' 2>&1; echo EXIT=\$?"; then
        if echo "$OUTPUT" | grep -qE "Read-only file system|Permission denied|EXIT=[1-9]"; then
            fail "D04: opt-out config did not restore writability" "$OUTPUT"
        else
            pass "D04: WRITABLE_TREE_RO_PATHS=() opt-out restores .git writability"
        fi
    else
        fail "D04: opt-out write produced unexpected error" "$OUTPUT"
    fi
    rm -f "$_opt_out_conf"
    if ! $_git_pre_existing2; then
        rm -rf "$_gitdir2"
    fi
else
    skip "D04: WRITABLE_TREE_RO_PATHS opt-out requires bwrap or firejail"
fi
